module cordic_serial(
    ...
    input signed [63:0]    x,       // cos值（64位）
    input signed [63:0]    y,       // sin值（64位）

    output reg   [31:0]    phase,   // 输出角度值，最大360*65536（保持32位）
...
wire    [31:0] rot[15:0];       // 角度查找表保持32位，匹配输出范围

// 旋转角度查找表（角度值放大到2^16级别，与原32位输出一致）
...
assign  rot[15] = 32'd128;      // ទ

// 定义上一个x和y的寄存器（改为64位）
reg signed [63:0] prev_x;
reg signed [63:0] prev_y;

reg signed [63:0] preprocessed_x;
reg signed [63:0] preprocessed_y;
reg               second_quadrant;
reg               third_quadrant;
reg               fourth_quadrant;
...
reg signed [63:0] x_shift;  // 内部计算使用64位
reg signed [63:0] y_shift;  // 内部计算使用64位
reg signed [31:0] z_rot;    // 角度累加保持32位，与输出一致

wire     D_sign;
assign   D_sign = ~y_shift[63];  // 取64位最高位符号
