...
(
    ...
    output reg signed [15:0] rx_data1,       // 接收数据1（有符号16位）
    output reg signed [15:0] rx_data2,       // 接收数据2（有符号16位）
    output reg signed [15:0] rx_data3,       // 接收数据3（有符号16位）
        ...
        case (state)
            ...
            STATE_3: begin
                ...
                if (rx_buffer[0] == PKT_HEAD && rx_buffer[7] == PKT_TAIL) begin
                    rx_data1 <= {rx_buffer[2], rx_buffer[1]}; // 高字节在前
                    rx_data2 <= {rx_buffer[4], rx_buffer[3]};
                    rx_data3 <= {rx_buffer[6], rx_buffer[5]};
                    rx_done <= 1'b1;
