...
(
    ...
    input wire [31:0] target_angles,    // 目标角度，32位宽度
...
parameter MAX_PULSE_PERIOD = 10000;    // 最大脉冲周期（最低速度）//10000
parameter MIN_PULSE_PERIOD = 5000;     // 最小脉冲周期（最高速度）//5000
parameter ACC_STEP         = 100;      // 每步增加或减少的脉冲周期 //100
    
parameter ANGLE_RATIO1  	= 3;        // 定义步进电机的角度比例
parameter STEPS_PER_REV 	= 6400;     // 每转一圈需要6400个脉冲（步距角1.8度）
    
reg                 move_flag;          // 标志位，指示电机是否正在运动
reg          [31:0] current_angles;     // 当前步进电机的角度，初始为0
...
parameter STATE_WAIT    = 2'b01;        // 使用IP核需要等一个时钟
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if (!sys_rst_n) begin
        ...
        case(state)
            STATE_CAL_ALL: begin  
                current_step <= 0;          // 重新下一个角度，复位清零             
                if (target_angles > current_angles) begin
                    // 如果目标角度大于当前角度，电机需要正转
                    // 13653 = ANGLE_RATIO1 * STEPS_PER_REV * 256 / 360
                    p <= 13653 * (target_angles - current_angles);
                    direction <= 1;  // 方向设为正转
                end
                else begin
                    // 如果目标角度小于当前角度，电机需要反转
                    // 13653 = ANGLE_RATIO1 * STEPS_PER_REV * 256 / 360
                    p <= 13653 * (current_angles - target_angles);
                    direction <= 0;  // 方向设为反转
                end
            end
            STATE_WAIT: begin
