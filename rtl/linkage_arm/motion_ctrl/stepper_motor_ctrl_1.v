...
parameter MAX_PULSE_PERIOD  = 6000;    // 最大脉冲周期（最低速度） //6000
parameter MIN_PULSE_PERIOD  = 3000;     // 最小脉冲周期（最高速度）//3000
parameter ACC_STEP 			 = 10;       // 每步增加或减少的脉冲周期 10

parameter ANGLE_RATIO1  	= 10;      // 定义平移步进电机的角度比例
        ...
        case(state)
            STATE_CAL_ALL: begin  
                ...
                if (target_angles > current_angles) begin
                    // 如果目标角度大于当前角度，电机需要正转
                    // 45511 = ANGLE_RATIO1 * STEPS_PER_REV * 256 / 360
                    p <= 45511 * (target_angles - current_angles);
                    direction <= 1;  // 方向设为正转
                end
                else begin
                    // 如果目标角度小于当前角度，电机需要反转
                    // 45511 = ANGLE_RATIO1 * STEPS_PER_REV * 256 / 360
                    p <= 45511 * (current_angles - target_angles);
                    direction <= 0;  // 方向设为反转
                end
            end
            STATE_WAIT: begin 
