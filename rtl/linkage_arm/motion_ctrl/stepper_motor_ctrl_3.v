...
module stepper_motor_ctrl_3
(
    input wire        sys_clk,         // 系统时钟输入，50MHz频率
    input wire        sys_rst_n,       // 复位信号输入，低电平复位
    input wire [31:0] target_angles,   // 目标角度，10位宽度，范围0到360
...
parameter ACC_STEP 			 = 2;       // 每步增加或减少的脉冲周期 2

parameter ANGLE_RATIO34 	= 7;       // 定义平移步进电机的角度比例
        ...
        case(state)
            STATE_CAL_ALL: begin  
                ...
                if (target_angles > current_angles) begin
                    // 如果目标角度大于当前角度，电机需要正转
                    //31857 = ANGLE_RATIO34 * STEPS_PER_REV * 256 / 360
                    p <= 31857 * (target_angles - current_angles);
                    direction <= 1;  // 方向设为正转
                end
                else begin
                    // 如果目标角度小于当前角度，电机需要反转
                    //31857 = ANGLE_RATIO34 * STEPS_PER_REV * 256 / 360
                    p <= 31857 * (current_angles - target_angles);
                    direction <= 0;  // 方向设为反转
                end
            end
            STATE_WAIT: begin 
