module sqrt_32 (
    ...
    input wire [31:0]   data,          // 需要开方的数
    output reg [15:0]   sqrt,          // 开方后的结果
    output reg          valid          // 输出结果有效标志
);

reg [31:0] radicand;                  // 被开方数
reg [15:0] result;                    // 开方结果
reg [15:0] bit;                       // 二进制位，用来进行逐位逼近
reg        busy;                      // 模块是否正在计算
reg        start;                     // 开始信号
reg [31:0] data_reg;                  // 存储上一次输入的 data 用于检测新数据
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if (!sys_rst_n) begin
        ...
        radicand <= 32'b0;
        start  <= 1'b0;
        data_reg <= 32'b0;            // 复位时清除 data_reg
    end
    else begin
        // 检测到新数据输入时，重新开始计算
        if (data != data_reg && !busy) begin
            radicand <= data;
            bit <= 16'h8000;           // 从最高位开始逼近
            result <= 16'b0;
            busy <= 1'b1;
